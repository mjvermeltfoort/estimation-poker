-- Follow-up migration for environments where 004 cannot be re-run.
-- Reapplies admin-role functions with idempotent, schema-driven role typing.
...
begin
  ...
  if jsonb_array_length(v_teams) = 0 then
    return jsonb_build_object(
      ...
      'members', '[]'::jsonb,
      'available_roles', jsonb_build_array('member', 'facilitator', 'admin')
    );
  end if;
  ...
  if v_role_type_kind = 'e' then
    select coalesce(
      ...
      jsonb_build_array('member', 'facilitator', 'admin')
    )
    ...
    from (
      ...
      union
      select unnest(array['member', 'facilitator', 'admin'])
    ) roles;
  end if;
...
begin
  ...
  v_role_input := coalesce(nullif(trim(coalesce(p_role, '')), ''), 'member');
  execute format('select ($1)::%s', pg_typeof(v_role)::text)
    into v_role
    using v_role_input;
...
begin
  ...
  if p_role is not null then
    if nullif(trim(p_role), '') is null then
      raise exception 'VALIDATION: role is required';
    end if;
    execute format('select ($1)::%s', pg_typeof(v_role)::text)
      into v_role
      using trim(p_role);
