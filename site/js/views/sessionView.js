...
function renderSession(app, model, viewer, roundNumber, context) {
  ...
  if (!ticket) {
    ...
    const titleRow = el("div", { className: "button-row" }, [
      el("h2", { text: ticket.jiraIssueKey || "Ticket" }),
      jiraUrl
        ? el("a", {
          ...
          "aria-label": `Open ${ticket.jiraIssueKey || "ticket"} in new tab`,
          text: "Open",
        }, [
    ...
    ticketPanel.append(
      el("div", { className: "ticket-heading" }, [el("div", {}, [titleRow]), statusBadge(ticket.status)]),
      ticket.projectName
